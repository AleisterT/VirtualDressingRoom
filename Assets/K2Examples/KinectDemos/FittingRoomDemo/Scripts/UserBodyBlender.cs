...
public class UserBodyBlender : MonoBehaviour 
{
	...
	[Range(-0.5f, 0.5f)]
	public float headThreshold = -0.1f;
...
//				userBlendMat.SetTexture("_ColorTex", colorTex);
	...
	void Update()
	{
		...
		if(kinectManager && kinectManager.IsInitialized())
		{
				...
				userBlendMat != null && lastDepthFrameTime != sensorData.lastDepthFrameTime)
			{
                ...
                var headPosition = GetComponent<Camera>().WorldToScreenPoint(headTransform.position);
                //var headPosition = GetComponent<Camera>().WorldToScreenPoint(kinectManager.GetJointPosition(0, (int) KinectInterop.JointType.Head));
                var headPositionNormalized = new Vector3(headPosition.x/Screen.width,headPosition.y/Screen.height,0);
                userBlendMat.SetVector("_HeadPosition",headPositionNormalized);
            }
		}
	}
	...
	void OnRenderImage (RenderTexture source, RenderTexture destination)
	{
		if(userBlendMat != null)
		{
			...
			userBlendMat.SetFloat("_HeadThreshold", headThreshold);
